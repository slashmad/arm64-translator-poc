...
while IFS= read -r LIB_ENTRY; do
    ...
    while [ "$attempt" -le "$ATTEMPTS" ]; do
        ...
        set +e
        if [ -n "$SMOKE_MAX_RETRIES" ]; then
            "$ROOT_DIR/scripts/run_kingshot_smoke.sh" "$APK_PATH" "$LIB_ENTRY" "" "$SMOKE_MAX_RETRIES" > "$TMP_OUT" 2>&1
            tiny_rc=$?
        else
            "$ROOT_DIR/scripts/run_kingshot_smoke.sh" "$APK_PATH" "$LIB_ENTRY" > "$TMP_OUT" 2>&1
            tiny_rc=$?
        fi
        set -e
        {
            echo "=== attempt $attempt/$ATTEMPTS rc=$tiny_rc ==="
            cat "$TMP_OUT"
        } >> "$LOG_FILE"
